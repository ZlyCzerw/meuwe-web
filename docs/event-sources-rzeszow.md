...
| WDK Rzeszów | https://wdk.kulturapodkarpacka.pl/ | ✅ INTEGRATED | HTML | Active custom source. Parses the homepage `Zapowiedzi`/`topic__date` block, keeps only `Zapowiedź wydarzenia`, and assigns events to WDK at `ul. S. Okrzei 7`; archive listing returned an empty body in curl, so the source intentionally uses the homepage. | Done |
| Podkarpacki Informator Kulturalny | https://kulturapodkarpacka.pl/ | ✅ INTEGRATED | HTML event cards | Active custom source. Parses dated `event-box` cards and emits only entries assignable to known Rzeszów venues from title/description text; added BWA, Muzeum Etnograficzne, and Plac Farny to the venue registry. | Done |
...
| Co Jest Grane Rzeszów | https://cojestgrane.pl/polska/podkarpackie/rzeszow | ✅ INTEGRATED | HTML + schema.org Event | Active custom source. Parses schema.org event cards with date, ticket hour(s), venue name, street address, category, source URL, image filtering, and duplicate ticketing-copy suppression; live scraper follows listing pagination. | Done |
